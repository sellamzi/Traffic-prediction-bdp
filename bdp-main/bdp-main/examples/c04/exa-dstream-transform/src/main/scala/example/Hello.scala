...
def main(args: Array[String]): Unit = {
    ...
    println("\n--- Spark Scala Streaming Word count example ---")
    ...
    val blacklist: RDD[(String, Unit)] = sc
      ...
      .map(word => (word, ()))
      .persist() // reused by every micro-batch!
    ...
    val filteredPairs: DStream[(String, Int)] = wordPairs.transform { _.subtractByKey(blacklist) }
    val wordCounts: DStream[(String, Int)] = filteredPairs.reduceByKey(_ + _)
